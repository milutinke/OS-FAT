...
public class SimpleDisk implements Disk {
	...
	public void writeSector(int sectorID, byte[] sectorData) throws DiskException {
		if (sectorData.length != sectorSize) {
			throw new DiskException("Error: Trying to write data of length: " + sectorData.length
					+ " to a disk with sectors of size: " + sectorSize);
		}
		if (sectorID < 0 || sectorID >= sectorCount) {
			throw new DiskException(
					"Error: Trying to write to sector: " + sectorID + " on a disk with " + sectorCount + " sectors.");
		}
	...
	public void writeSectors(int startingSector, int count, byte[] sectorData) throws DiskException {
		if (sectorData.length != count * sectorSize) {
			throw new DiskException("Error: Trying to write data of length: " + sectorData.length + " to " + count
					+ " sectors on a disk with sectors of size: " + sectorSize);
		}
		if (startingSector < 0 || startingSector + count > sectorCount) {

			throw new DiskException("Error: Trying to write " + count + " sectors to position: " + startingSector
					+ " on a disk with " + sectorCount + " sectors.");
		}
	...
	public byte[] readSector(int sectorID) throws DiskException {
		if (sectorID < 0 || sectorID >= sectorCount) {
			throw new DiskException(
					"Error: trying to read sector " + sectorID + " on a disk with " + sectorCount + " sectors.");
		}
	...
	public byte[] readSectors(int startingSector, int count) throws DiskException {
		if (startingSector < 0 || startingSector + count > sectorCount) {
			throw new DiskException("Error: trying to read " + count + " sectors from position " + startingSector
					+ " on a disk with " + sectorCount + " sectors.");
		}
